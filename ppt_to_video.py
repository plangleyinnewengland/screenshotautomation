...
def _tts_worker_process(text: str, output_path: str, rate: int, volume: float, voice_id: Optional[str]):
    """Run pyttsx3 in a separate process so hung TTS calls can be terminated safely."""
    try:
        import pyttsx3
...
class PPTToVideoConverter:
    ...
    def _generate_audio(self, text: str, output_path: str) -> Optional[str]:
        ...
        try:
            ...
            if process.exitcode not in (0, None):
                print(f"Warning: TTS process exited with code {process.exitcode}")
                return None
    ...
    def convert(
        ...
        try:
            ...
            # Step 6: Export video with proper finalization
            ...
            try:
                ...
                # Ensure file was created and has content
                ...
                # Clean up all clips and audio resources
                final_clip.close()
                for clip in clips:
                    try:
                        clip.close()
